...
pub async fn create_profile_upload(
    ...
    let file_size = payload.get("file_size").and_then(|v| v.as_i64());
...
pub async fn get_user_profile_uploads(
    ...
    let uploads =
        services::profile::get_user_profile_uploads(&state.profile_repo, ctx.user_id()).await?;
...
pub async fn set_current_avatar(
    ...
    let upload =
        services::profile::get_upload_by_id(&state.profile_repo, payload.profile_upload_id)
            .await?
            .ok_or_else(|| crate::error::Error::NotFound {
                message: "Profile upload not found".to_string(),
            })?;
...
pub async fn delete_profile_upload(
    ...
    services::profile::delete_profile_upload(&state.profile_repo, ctx.user_id(), upload_id).await?;
